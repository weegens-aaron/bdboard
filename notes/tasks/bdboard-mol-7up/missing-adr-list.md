...
> `notes/tasks/bdboard-mol-c09/missing-adr-list.md` (2026-05-29). It (a)
...
2. Re-inventoried the written decision homes:
   - `notes/decisions/` — formal ADRs. **Still only one:**
     `0002-dashboard-architecture.md` (unchanged since the c09 audit).
   - `notes/design/<bead-id>/` — 8 design docs + 2 `*-decision.md` files.
...
**Nothing from the previous audit was acted on.** `notes/decisions/` is byte-for-byte
...
All four still have **no** durable record in `notes/decisions/`. Restated tersely;
...
| M4 | Manual field-editing model (registry, open-only, append-only, optimistic-lock) | `notes/design/bdboard-7q9/` spike + bead notes | STILL MISSING |

Process gaps also unchanged:

- **X1** — no `notes/decisions/0001-*.md`; numbering still starts at `0002`.
- **X2** — still two competing decision homes (`notes/decisions/*.md` vs
  `notes/design/<bead>/*-decision.md`) with no rule for which to use, and no ADR
...
- **Why it's ADR-worthy (borderline):** This **reverses** an earlier, documented
  design — `notes/design/bdboard-ace/cadence-invoker-decision.md` and the
...
- **Recommended:** lightweight — add a "Superseded by bdboard-65z (formulas are
  now variable-less)" note to `notes/design/bdboard-ace/cadence-invoker-decision.md`
