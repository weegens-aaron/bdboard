...
# ----- coordination_count (nav badge, bdboard-iz8h) -----


def _slot(*, held: bool = False, waiter_count: int = 0) -> dict:
    """Minimal merge_slot_view-shaped dict for badge counting."""
    return {
        "id": "x-merge-slot",
        "held": held,
        "holder": "agent-7" if held else None,
        "waiters": [f"w{i}" for i in range(waiter_count)],
        "waiter_count": waiter_count,
        "state": "held" if held else "available",
    }


def test_coordination_count_zero_when_quiet():
    """No gates and no slots -> 0 (badge renders nothing)."""
    assert derive.coordination_count([], []) == 0


def test_coordination_count_counts_every_open_gate():
    """Each open gate contributes 1 (bd gate list returns only OPEN gates)."""
    gates = [{"id": "gt-1"}, {"id": "gt-2"}, {"id": "gt-3"}]
    assert derive.coordination_count(gates, []) == 3


def test_coordination_count_held_slot_counts():
    """A held merge slot is something to look at -> counts."""
    assert derive.coordination_count([], [_slot(held=True)]) == 1


def test_coordination_count_contended_available_slot_counts():
    """An available-but-contended slot (waiters queued) counts."""
    assert derive.coordination_count([], [_slot(held=False, waiter_count=2)]) == 1


def test_coordination_count_idle_available_slot_is_excluded():
    """An available slot with no waiters is idle infrastructure -> excluded."""
    assert derive.coordination_count([], [_slot(held=False, waiter_count=0)]) == 0


def test_coordination_count_combines_gates_and_relevant_slots():
    """Total = open gates + held/contended slots; idle slots drop out."""
    gates = [{"id": "gt-1"}, {"id": "gt-2"}]
    slots = [
        _slot(held=True),  # +1
        _slot(held=False, waiter_count=3),  # +1 (contended)
        _slot(held=False, waiter_count=0),  # +0 (idle)
    ]
    assert derive.coordination_count(gates, slots) == 4


def test_coordination_count_tolerates_missing_slot_keys():
    """A defensively-shaped slot dict (missing keys) never raises."""
    assert derive.coordination_count([], [{}]) == 0
    assert derive.coordination_count([], [{"held": True}]) == 1
    assert derive.coordination_count([], [{"waiter_count": 1}]) == 1
